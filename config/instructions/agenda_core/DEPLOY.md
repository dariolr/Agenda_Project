...
mysql -u agenda_user -p agenda_core_prod < /var/www/config/migrations/FULL_DATABASE_SCHEMA.sql

# Seed solo se necessario (dati demo)
# mysql -u agenda_user -p agenda_core_prod < /var/www/config/migrations/seed_data.sql
```
...
- `config/migrations/` — file SQL (usare solo per setup iniziale)
