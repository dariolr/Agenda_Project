...
/*
 * Single-file demo reset+seed runner for cron.
 * Required env vars: DB_DATABASE, DB_USERNAME
 * Optional env vars: DB_HOST (127.0.0.1), DB_PORT (3306), DB_PASSWORD ('')
 */
...
if ($dbName === '' || $dbUser === '') {
    fwrite(STDERR, "ERROR: DB_DATABASE and DB_USERNAME are required\n");
    exit(1);
}
...
echo '[demo-cron-php] reset start: ' . date('Y-m-d H:i:s') . PHP_EOL;
$pdo->exec(SQL_RESET);

echo '[demo-cron-php] seed start' . PHP_EOL;
$pdo->exec(SQL_SEED);

echo '[demo-cron-php] done: ' . date('Y-m-d H:i:s') . PHP_EOL;

const SQL_RESET = <<<'SQL_RESET'
